...
async function audit() {
      ...
      try {
        ...
        const legacyColors = await page.evaluate((fire) => {
          ...
          document.querySelectorAll('*').forEach((el) => {
            ...
            ;['color', 'backgroundColor', 'borderTopColor', 'borderRightColor', 'borderBottomColor', 'borderLeftColor'].forEach((p) => {
              if (legacy.includes(cs[p])) {
                found.push(`${el.tagName.toLowerCase()}.${(el.className || '').toString().slice(0, 40)} ${p}=${cs[p]}`)
              }
            })
          })
        ...
        if (legacyColors.length) {
          issues.push({ severity: 'high', kind: 'legacy-color', msg: `cores legadas: ${legacyColors.join(' | ')}` })
        }
        ...
        const btnTextColors = await page.evaluate(() => {
          const btns = Array.from(document.querySelectorAll('.btn-primary, .btn-fire, button, a')).filter((el) => {
            const cs = getComputedStyle(el)
            const bg = cs.backgroundColor
            // Acid-ish background or classes
            return bg === 'rgb(198, 255, 0)' || (el.className || '').toString().includes('btn-primary') || (el.className || '').toString().includes('bg-acid')
          })
          return btns.slice(0, 5).map((b) => {
            const cs = getComputedStyle(b)
            return { class: (b.className || '').toString().slice(0, 60), color: cs.color, bg: cs.backgroundColor, text: (b.textContent || '').trim().slice(0, 30) }
          })
        })
        btnTextColors.forEach((b) => {
          // Wrong: text white/cream on acid bg (should be ink #050505)
          if ((b.bg === 'rgb(198, 255, 0)' || b.class.includes('btn-primary') || b.class.includes('bg-acid')) && (b.color === 'rgb(255, 255, 255)' || b.color === 'rgb(245, 241, 232)')) {
            issues.push({ severity: 'high', kind: 'button-contrast', msg: `botão acid com texto claro: "${b.text}" color=${b.color}` })
          }
        })

        // 4. Check emoji facial (forbidden)
        const emojiIssues = await page.evaluate(() => {
          const forbidden = /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F900}-\u{1F9FF}\u{1F1E0}-\u{1F1FF}\u{2700}-\u{27BF}]/u
          // Allowed: ★ → ▶ ● ▲ ▼
        ...
        if (emojiIssues.length) {
          issues.push({ severity: 'medium', kind: 'emoji-forbidden', msg: emojiIssues.join(' | ') })
        }
        ...
        if (hasHScroll) {
          issues.push({ severity: 'high', kind: 'h-scroll', msg: `documentElement.scrollWidth > viewport` })
        }
        ...
        results.push({ viewport: vpName, route, status, issues: [{ severity: 'high', kind: 'error', msg: String(e.message || e) }], screenshot: null })
  ...
  const byKind = {}
  results.forEach((r) => r.issues.forEach((i) => { byKind[i.kind] = (byKind[i.kind] || 0) + 1 }))
  console.log('\n====== SUMMARY ======')
